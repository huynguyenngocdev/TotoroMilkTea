    ...
    callAPI(endpoint, "GET", null)
      .then((res) => {
        ...
        if (check.length === 0) {
          ...
          $("#incorrect-email")
            .css("color", "red")
            .html("Error: Email này chưa đăng ký!");
        } else {
          let u = check[0].username;
          let p = check[0].password;
          $("#incorrect-email")
            .css("color", "#20c997")
            .html("Notice: Email chính xác! ");
          let content = `
          ...
          SendEmail(email, content)
            .then(() => {
              ...
              $("#closeForgotPwModal").click();
            })
            .catch(() =>
              alert("Gửi mail thất bại vui lòng kiểm tra đường truyền!")
            );
        }
      })
      .catch((err) => {
